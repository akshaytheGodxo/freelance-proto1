...
export const freelancerRouter = createTRPCRouter({
  ...
  getMyProjects: publicProcedure
    .input(z.object({ freelancerEmail: z.string() }))
    .query(async ({ input }) => {
      const freelancer = await db.user.findUnique({
      ...
      if (!freelancer?.freelancer) throw new Error("Freelancer not found");
  ...
  markMilestoneComplete: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        return { success: true, message: "Milestone marked as completed ✅", milestone: updatedMilestone };
      } catch (error) {
